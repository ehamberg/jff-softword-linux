...
bool CreateGLWindow(const char* title, int width, int height, int bits, bool fullscreenflag)
{
    ...
    Uint32 SDL_VIDEO_FLAGS = SDL_OPENGL;

    if (fullscreenflag)
        SDL_VIDEO_FLAGS += SDL_FULLSCREEN;

    screen = SDL_SetVideoMode(width, height, bits, SDL_VIDEO_FLAGS);
...
int main(int argc, char* argv[])
{
    ...
    for (int i = 1; i < argc; i++) {
        if (strcmp(argv[i], "--resx") == 0 && argc > i+1) {
            i++;
            res_x = atoi(argv[i]);
            continue;
        }
        if (strcmp(argv[i], "--resy") == 0 && argc > i+1) {
            i++;
            res_y = atoi(argv[i]);
            continue;
        }
        ...
        std::cerr << "\t--resx XRESOLUTION\t- set resolution (width)" << std::endl;
        std::cerr << "\t--resy YRESOLUTION\t- set resolution (height)" << std::endl;
        std::cerr << "\t--windowed\t\t- set window mode" << std::endl;
    ...
    if (!FSOUND_Init(44100, 0)) {
        std::cerr << "Unable to initialize fmod (sound driver in use?)" << std::endl;
        KillGLWindow();
        return 1;
    }
    // LOAD SONG
    ...
    if (!mod)   {
        std::cerr << "Unable to load song" << std::endl;
        KillGLWindow();
        return 1;
    }
    // PLAY SONG
        ...
        if (active)                             // Program Active?
        {
            ...
            else
            {
                ...
                if ((ord>=4) && (ord<8))
                {
                    ...
                    X=SDL_GetTicks()/50.0;
                    deform_cyl2=SDL_GetTicks()/1000.0;
                    deform_cyl3=SDL_GetTicks()/1000.0;
                ...
                else
                {
                    ...
                    if ((ord>=8) && (ord<10))
                    {
                        ...
                        X=SDL_GetTicks()/50.0;
                        deform_cyl2=(float)(SDL_GetTicks()/2000.0);
                        deform_cyl3=(float)(SDL_GetTicks()/2000.0);
                        glClearColor(c_fond[0],c_fond[1],c_fond[2], 0.0f);
                        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
                        glEnable(GL_FOG);
                        Affiche_Part3();
